...
$webDriver.getCapabilities().then(function () { })
    ...
    .then(function () {
        return Promise.resolve(true)
            ...
            .then(function () { return logger.log(7, "Click By.id(\"signInAnotherWay\")", "mfa"), $browser.waitForAndFindElement($selenium.By.id("signInAnotherWay"), DefaultTimeout).then(e => (e.click(), Promise.resolve(!0))) })  
            .then(function () { return logger.log(8, "Click By.xpath('//*[@id=idDiv_SAOTCS_Proofs]/div[2]/div/div/div[2]/div')", "mfa"), $browser.waitForAndFindElement($selenium.By.xpath('//*[@id="idDiv_SAOTCS_Proofs"]/div[2]/div/div/div[2]/div'), DefaultTimeout).then(e => (e.click(), Promise.resolve(!0))) })  
            .then(function () { return logger.log(9, "Enter MFA token using locator By.id(\"idTxtBx_SAOTCC_OTC\")", "mfa"), $browser.waitForAndFindElement($selenium.By.id("idTxtBx_SAOTCC_OTC"), DefaultTimeout).then(e => (e.sendKeys(totp.generate()), Promise.resolve(!0))) })
            .then(function () { return logger.log(10, "Click By.id(\"idSubmit_SAOTCC_Continue\")", "mfa"), $browser.waitForAndFindElement($selenium.By.id("idSubmit_SAOTCC_Continue"), DefaultTimeout).then(e => (e.click(), Promise.resolve(!0))) })
            .then(function () { return logger.log(11, "Sleep a little", "M365"), $browser.sleep (1000) })
            .then(function () { return logger.log(12, "Click By.id(\"idBtn_Back\")", "M365"), $browser.waitForAndFindElement($selenium.By.id("idBtn_Back"), DefaultTimeout).then(e => (e.click(), Promise.resolve(!0))) })
            .then(function () { return logger.log(13, "Sleep a little", "M365"), $browser.sleep (1000) })
            //.then(function () { return logger.log(14, "Click By.id(\"welcome-title\")", "M365"), $browser.waitForAndFindElement($selenium.By.id("welcome-title"), DefaultTimeout) })
            .then(function () {
